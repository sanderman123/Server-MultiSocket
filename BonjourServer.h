...
- (void)serverRemoteConnectionComplete:(BonjourServer *)server;
// called when the server is finished stopping
- (void)serverStopped:(BonjourServer *)server;
// called when something goes wrong in the starup
- (void)server:(BonjourServer *)server didNotStart:(NSDictionary *)errorDict;
// called when data gets here from the remote side of the server
- (void)server:(BonjourServer *)server didAcceptData:(NSData *)data;
// called when the connection to the remote side is lost
- (void)server:(BonjourServer *)server lostConnection:(NSDictionary *)errorDict;
...
@interface BonjourServer : NSObject <NSNetServiceDelegate, NSStreamDelegate, NSNetServiceBrowserDelegate>
{
